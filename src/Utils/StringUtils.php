...
class StringUtils
{
     ...
     */
    public static function removeEmoji($string, bool $trim = true)
    {
        // Keep old regex for reference
        ...
        $regsEmoji = [
            '/[(\x{1F600}-\x{1F64F})|(\x{2700}-\x{27BF})|(\x{1F680}-\x{1F6FF})|(\x{24C2}-\x{1F251})|(\x{1F30D}-\x{1F567})|(\x{1F900}-\x{1F9FF})|(\x{1F300}-\x{1F5FF})]/mu',
            '/\x{200D}*/mu',
        ];
        $string    = preg_replace($regsEmoji, '', $string);

        return $trim ? trim($string) : $string;
    }

    /**
     * Convert an array of hex strings into an unicode utf8 string
     *
     * @param array $unicodeHex
     * @return string
     */
    public static function unicodeHexToString(array $unicodeHex): string
    {
        $str = '';
        foreach ($unicodeHex as $h) {
            $str .= mb_convert_encoding('&#' . hexdec($h) . ';', 'UTF-8', 'HTML-ENTITIES');
        }
    ...
    /**
     * Search a JSON/js in a string and closes the object when all brackets matches
     *
     * @param $jsString
     * @param $search
     * @return mixed
    ...
    /**
     * Mimics Javascript encodeURIComponent
     *
     * @param $str
     * @return string
     */
